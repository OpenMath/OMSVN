...
<xsl:stylesheet  version="1.0"
  ...
  <xsl:template match="omcd:Signature">
    <xsl:param name="cd"/>
    <xsl:variable name="elt" select="concat(@name,'@',$cd)"/>
  ...
  <xsl:template match="om:OMS" mode="rnc">
    <xsl:param name="elt"/>
    <xsl:value-of select="concat(@name,' | = ',$elt,'&#xA;')"/>
  </xsl:template>
  ...
  <xsl:template match="om:OMS" mode="type">
    <xsl:value-of select="concat(@name,'@',@cd)"/>
  </xsl:template>

  <xsl:template match="om:OMA[om:OMS[1 and @cd='sts' and @name='nassoc']]" mode="type">
