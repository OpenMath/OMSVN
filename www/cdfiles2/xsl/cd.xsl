...
<dd><xsl:for-each select="cd:CDDefinition/cd:Name">
  ...
  <xsl:for-each select="cd:CDUses/cd:CDName">
      <xsl:variable name="ocdFileExists"><xsl:call-template name="test-file-exists"><xsl:with-param name="name"><xsl:value-of select="."/>.ocd</xsl:with-param></xsl:call-template></xsl:variable>
