...
<xsl:variable name="extra" select="document('index.xml',.)"/>
<xsl:variable name="contrib" select="document('../cdfiles/contrib/log.xml',.)"/>
...
<CD path="../cdfiles/contrib" name="{@name}">
...
<td><pre><xsl:copy-of select="(Description|cd:Description)/node()"/></pre></td>
</tr>
</xsl:if>
</xsl:for-each>
</table>
