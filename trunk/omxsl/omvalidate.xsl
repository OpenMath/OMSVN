...
exclude-result-prefixes="xlink saxon om">
...
<xsl:variable name="x">
<xsl:if test="$om-nons">
<p>OpenMath should be in the namespace http://www.openmath.org/OpenMath</p>
</xsl:if>

 <xsl:if test="not($om)">
<p>No OpenMath found!</p>
</xsl:if>
...
<xsl:template match="*">
  <p>Unknown element <xsl:value-of select="name()"/></p>
</xsl:template>

<xsl:template match="om:OMS">
 <xsl:variable name="om2" select="ancestor::om:OMOBJ[1]/@version &gt;= 2.0"/>
 <xsl:if test="not(@name)">
  <p>OMS must have a name attribute</p>
 </xsl:if>
 <xsl:if test="not(@cd)">
  <p>OMS must have a cd attribute</p>
 </xsl:if>
 <xsl:choose>
 <xsl:when test="$om2
    and @*[not(name()='name' or name()='cd' or name()='cdbase')]">
  <p>Bad attribute on OMS:
...
<xsl:template match="om:OMV">
 ...
 <p> OMV must have just name attribute:
<xsl:for-each select="@*[not(name()='name')]">
  <xsl:value-of select="name()"/>="<xsl:value-of select="."/>" 
</xsl:for-each></p>
 </xsl:if>
 <xsl:if test="* or not(. = '')">
 <p> OMV must be EMPTY</p>
 </xsl:if>
<xsl:if test="not($om2) and translate(@name,&quot;abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789+=(),-\./:?!#$%*;@\[\]\^_`{|}&quot;,'') != ''">
<p>Illegal character in OMV name="<xsl:value-of select="@name"/>"</p>
 </xsl:if>
</xsl:template>

<xsl:template match="om:OMB|om:OMSTR">
 <xsl:if test="@*">
 <p><xsl:value-of select="local-name()"/> does not take attributes</p>
 </xsl:if>
 <xsl:if test="*">
 <p><xsl:value-of select="local-name()"/> should only have character data</p>
 </xsl:if>
</xsl:template>

<xsl:template match="om:OMI">
 <xsl:if test="@*">
 <p>OMI does not take attributes</p>
 </xsl:if>
 <xsl:if test="*">
...
<xsl:template match="om:OMF">
 ...
 <p>  OMF must only have dec or hex attribute</p>
 </xsl:if>
 <xsl:if test="* or not(. = '')">
...
<xsl:if test="translate(@hex,'ABCDEF0123456789 ','') != ''">
 <p>Illegal character in OMF hex attribute: <xsl:value-of select="@hex"/></p>
</xsl:if>
</xsl:template>
...
<xsl:variable name="x">
 ...
 <xsl:when test="$om2 and @*[not(name()='cdbase' or name()='version')]">
  <p>OMOBJ does not take attributes</p>
 </xsl:when>
 ...
 <xsl:if test="not(*)">
  <p>OMOBJ must have at least one child element</p>
 </xsl:if>
 <xsl:apply-templates/>
</xsl:variable>
<xsl:if test="string($x)">
<p>Errors in OMOBJ <xsl:value-of select="position()"/>
...
<xsl:template match="om:OMA">
 <xsl:if test="@*">
  <p>OMA does not take attributes</p>
 </xsl:if>
 <xsl:if test="not(*)">
  <p>OMA must have at least one child element</p>
 </xsl:if>
 <xsl:apply-templates/>
</xsl:template>
...
<xsl:template match="om:OME">
 <xsl:if test="@*">
  <p>OME does not take attributes</p>
 </xsl:if>
 <xsl:if test="not(*[1][self::om:OMS])">
  <p>OME must have first child an OMS</p>
 </xsl:if>
 <xsl:apply-templates/>
</xsl:template>
...
<xsl:template match="om:OMBVAR">
 <xsl:if test="@*">
  <p>OMBVAR does not take attributes</p>
 </xsl:if>
 <xsl:apply-templates mode="var"/>
</xsl:template>

<xsl:template mode="var" match="node()">
 <p>OMBVAR may only contain variables and attributed variables: <xsl:value-of select="local-name()"/></p>
</xsl:template>
...
<xsl:template mode="var" match="om:OMATTR">
 <xsl:if test="not(count(*)=2)">
  <p>OMATTR must have exactly two children</p>
 </xsl:if>
...
<xsl:template match="om:OMATP">
 <xsl:if test="@*">
  <p>OMATP does not take attributes</p>
 </xsl:if>
...
<xsl:template match="om:OMFOREIGN">
 ...
 <xsl:when test="not($om2)">
  <p>OMFOREIGN Not Allowed in Openmath 1</p>
 </xsl:when>
 <xsl:otherwise>
...
<xsl:template match="om:OMR">
 ...
 <xsl:choose>
 <xsl:when test="not($om2)">
  <p>OMR Not Allowed in Openmath 1</p>
 </xsl:when>
 <xsl:otherwise>
  <xsl:if test="* or not(. = '')">
  <p>OMR should be EMPTY</p>
 </xsl:if>
 <xsl:if test="not(@xlink:type='simple')">
   <p>OMR mmust have xlink:type="simple"</p>
 </xsl:if>
 <xsl:if test="not(@xlink:show='embed')">
   <p>OMR mmust have xlink:type="embed"</p>
 </xsl:if>
 <xsl:if test="not(@xlink:href)">
   <p>OMR mmust have an xlink:href attribute</p>
 </xsl:if>
 </xsl:otherwise>
 </xsl:choose>
</xsl:template>
